...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (empty($_POST["yoa"])) {
     ...
     if (!preg_match("/^[0-9]{4}$/",$yoa)) {$flag=0;
    ...
    if (empty($_POST["e_id"])) {
     ...
     if (!filter_var($e_id, FILTER_VALIDATE_EMAIL)) {$flag=0;
...
if (empty($_POST["m_id"])) {
     ...
     if (!filter_var($m_id, FILTER_VALIDATE_EMAIL)) {$flag=0;
   ...
   if (empty($_POST["f_id"])) {
    ...
    if (!filter_var($f_id, FILTER_VALIDATE_EMAIL)) {$flag=0;
   ...
   if (empty($_POST["rank"])) {
     ...
     // check if name only contains letters and whitespace
     if (!preg_match("/^[0-9]{1,6}$/",$rank)) {$flag=0;
       $rankErr = "invalid"; 
     }
   }
   ...
   if (empty($_POST["ph"])) {
     ...
     // check if name only contains letters and whitespace
     if (!preg_match("/^[7-9]{1}+[0-9]{9}$/",$ph)) {$flag=0;
   ...
   if (empty($_POST["f_name"])) {
     ...
     if (!preg_match("/^[a-zA-Z ]*$/",$f_name)) {$flag=0;
   ...
   if (empty($_POST["f_no"])) {
     ...
     // check if name only contains letters and whitespace
     if (!preg_match("/^[7-9]{1}+[0-9]{9}$/",$f_no)) {$flag=0;
   ...
   if (empty($_POST["m_name"])) {
     ...
     // check if name only contains letters and whitespace
     if (!preg_match("/^[a-zA-Z ]*$/",$m_name)) {$flag=0;
   ...
   if (empty($_POST["m_no"])) {
     ...
     // check if name only contains letters and whitespace
     if (!preg_match("/^[7-9]{1}+[0-9]{9}$/",$m_no)) {$flag=0;
       $m_noErr = "invalid no"; 
     }
   }
